...
const ScheduleAppointment = ({ open, onClose, onSuccess }) => {
    ...
    useEffect(() => {
        if (!open) {
            ...
            const fetchDoctors = async () => {
                try {
                    const response = await axios.get("http://localhost:8000/api/public/doctors");
    ...
    const handleBookNow = async (e) => {
        ...
        try {
            const response = await axios.post("http://localhost:8000/api/public/appointment", formData);
                            ...
                            >
                                ...
                                {doctors.map((doctor) => (
                                    <MenuItem key={doctor._id || doctor.id} value={doctor._id || doctor.id}>
                                        {doctor.firstName} {doctor.lastName} {doctor.specialization || doctor.specialty ? `- ${doctor.specialization || doctor.specialty}` : ""}
                                    </MenuItem>
                                ))}
                            </TextField>
                        </Box>
